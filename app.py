...
def bloomberg():
    ...
    except: return [("Bloomberg: Dollar Softens, Gold Bid",""), ("Fed Rate Cut Bets","")]
...
""", unsafe_allow_html=True)
st.write(f"SAST Time: {now.strftime('%H:%M:%S')}")
...
k2.markdown(f'<div class="kpi"><div class="kpi-label">DXY FUND</div><div class="kpi-val">{dxy:.2f}</div><div style="color:{"#00e676" if dxy_chg<0 else "#ff5252"};font-size:12px;">{dxy_chg:+.2f}%</div></div>', unsafe_allow_html=True)
k3.markdown(f'<div class="kpi"><div class="kpi-label">TREND</div><div class="kpi-val">{"BULL" if ema50>ema200 else "BEAR"}</div><div style="color:#888;font-size:12px;">{ema50:.0f}/{ema200:.0f}</div></div>', unsafe_allow_html=True)
session_name = "LONDON" if 10<=now.hour<13 else "NY" if 15<=now.hour<20 else "ASIA"
k4.markdown(f'<div class="kpi"><div class="kpi-label">SESSION</div><div class="kpi-val">{session_name}</div><div style="color:{"#00e676" if 10<=now.hour<20 else "#ff9800"};font-size:12px;">{"HIGH" if 10<=now.hour<20 else "LOW"}</div></div>', unsafe_allow_html=True)
k5.markdown(f'<div class="kpi"><div class="kpi-label">DISCIPLINE</div><div class="kpi-val">{len(st.session_state.trades)}/2</div><div style="color:{"#ff5252" if len(st.session_state.trades)>=2 else "#00e676"};font-size:12px;">{"LOCKED" if len(st.session_state.trades)>=2 else "READY"}</div></div>', unsafe_allow_html=True)

left, right = st.columns([1.7, 1])
with left:
    st.markdown('<div class="glass" style="margin-top:15px;"><div class="kpi-label">LIVE CHART</div>', unsafe_allow_html=True)
    ...
    if len(st.session_state.trades)>=2:
        st.markdown(f'<div class="locked"><h3>🔒 LIMIT 2/2 DONE</h3><p>{", ".join(st.session_state.trades)}</p></div>', unsafe_allow_html=True)
    elif agree_buy:
        sl=price-atr*1.5; tp=price+(abs(price-sl)*2.5)
        st.markdown(f'<div class="buy-signal">🟢 ELITE BUY<br><span style="font-size:14px;">{price:.2f} | SL {sl:.2f} | TP {tp:.2f}</span></div>', unsafe_allow_html=True)
        if st.button("✅ EXECUTE BUY"): st.session_state.trades.append(f"BUY {now.strftime('%H:%M')} {price:.2f}"); st.rerun()
    elif agree_sell:
        sl=price+atr*1.5; tp=price-(abs(sl-price)*2.5)
        st.markdown(f'<div class="sell-signal">🔴 ELITE SELL<br><span style="font-size:14px;">{price:.2f} | SL {sl:.2f} | TP {tp:.2f}</span></div>', unsafe_allow_html=True)
        if st.button("✅ EXECUTE SELL"): st.session_state.trades.append(f"SELL {now.strftime('%H:%M')} {price:.2f}"); st.rerun()
    else:
        st.markdown(f'<div class="wait-signal"><h3>⚪ STOIC WAIT</h3><p style="color:#888;">Need Agreement - Session OK: {session_ok}</p></div>', unsafe_allow_html=True)

with right:
    st.markdown('<div class="glass"><div class="kpi-label">RISK TERMINAL</div>', unsafe_allow_html=True)
    bal = st.number_input("Balance $", 100.0, 50000.0, 500.0)
    ...
    st.markdown(f'<div style="margin-top:12px;background:#111;border-radius:10px;padding:12px;">Risk <span class="gold-text">${risk_amt:.2f}</span> → Reward ${risk_amt*rr_v:.2f}<br>Lot <span class="gold-text">{lots:.2f}</span></div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('<div class="glass" style="margin-top:15px;"><div class="kpi-label">BLOOMBERG FEED</div>', unsafe_allow_html=True)
    for title, link in news:
        st.markdown(f'<div style="background:#111;padding:10px;border-radius:8px;margin:6px 0;border-left:3px solid #FFD700;font-size:12px;">📰 {str(title)[:110]}</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="glass" style="margin-top:15px;"><div class="kpi-label">JOURNAL {len(st.session_state.trades)}/2</div>', unsafe_allow_html=True)
    ...
    else: st.caption("No trades today")
    st.markdown('</div>', unsafe_allow_html=True)
